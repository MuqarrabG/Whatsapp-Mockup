      ...
      {chats.map((chat, index) => (
        ...
        >
          ...
          <div className="chat-info flex-grow">
            ...
            <p className="chat-last-message text-gray-700 text-sm">{chat.lastMessage.sender+": "} {chat.lastMessage.message}</p>
          </div>
          <div className="chat-time mr-auto">
            <p className="text-gray-500 text-sm">{formatTimestamp(chat.timestamp)}</p>
          </div>
        </button>
      ))}
    </div>
